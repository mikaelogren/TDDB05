...
/*Need to get what state we're in by looking at which the five state parameters.
 * Somehow we need to convert them to an integer value.
 * Then without using a big switch or something like that we need to call the corresponding set-function
 * Then we need to set that state to the value given. 
 * 
 * getMethod(String name) är intressant. Returnar en Method object för en metod som heter name 
*/
	public  void setValue( String sV ) {
		String BinState = getBinState();
		int IntState = Integer.parseInt(BinState, 2);
		String MethodName = "setV".concat(Integer.toString(IntState));
		System.out.println(MethodName);
		try{
			Method SetFunc = this.getClass().getMethod(MethodName, String );
			SetFunc.invoke(this, sV);
			System.out.println(SetFunc.getName());
		}catch (SecurityException e) {
			System.out.println("SecurityException");
		}catch (NoSuchMethodException e) {
			System.out.println("NoSuchMethodException");
		}catch (IllegalArgumentException e) {
			System.out.println("IllegalAgrumentException");
	...
	private String getBinState(){
		
		String s = "";
		
		s=s.concat(Integer.toString(getP0()));
		s=s.concat(Integer.toString(getP1()));
		s=s.concat(Integer.toString(getP2()));
		s=s.concat(Integer.toString(getP3()));
		s=s.concat(Integer.toString(getP4()));
		return s;
